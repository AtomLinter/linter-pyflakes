...
export default {
  activate() {
    ...
    this.subscriptions.add(
      atom.config.observe('linter-pyflakes.pyflakesExecutablePath', (value) => { this.execPath = value; }),
      atom.config.observe('linter-pyflakes.excludePatterns', (value) => { this.excludePatterns = value; }),
    );
  },
    ...
    return {
      ...
      lint: async (textEditor) => {
        ...
        if (this.excludePatterns.length > 0) {
          parameters.push(`--exclude=${this.excludePatterns.join(',')}`);
        }
        parameters.push('-');
        ...
        const result = await helpers.exec(this.execPath, parameters, execOpts);
